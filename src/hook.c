...
void		moving(int keycode, t_args *args)
{
	...
	else if (keycode == FORWARD | keycode == BACK)
		args->env->hook.backforw = (keycode == FORWARD ? 1 : -1);
	else if ((keycode == C_SCALE || keycode == V_SCALE) && SEL != 0)
		args->env->hook.scale = (keycode == C_SCALE ? 1 : -1);
	else if ((keycode == Q_TRANSX || keycode == A_TRANSX) && SEL != 0)
		args->env->hook.trans_x = (keycode == Q_TRANSX ? 1 : -1);
	else if ((keycode == W_TRANSY || keycode == S_TRANSY) && SEL != 0)
		args->env->hook.trans_y = (keycode == W_TRANSY ? 1 : -1);
	else if ((keycode == E_TRANSZ || keycode == D_TRANSZ) && SEL != 0)
		args->env->hook.trans_z = (keycode == E_TRANSZ ? 1 : -1);
	else if ((keycode == R_ROTX || keycode == F_ROTX) && SEL != 0)
		args->env->hook.rot_x = (keycode == R_ROTX ? 1 : -1);
	else if ((keycode == T_ROTY || keycode == G_ROTY) && SEL != 0)
		args->env->hook.rot_y = (keycode == T_ROTY ? 1 : -1);
	else if ((keycode == Y_ROTZ || keycode == H_ROTZ) && SEL != 0)
